...
void *(vg_init)(uint16_t mode) {
  ...
  second_buffer = (char*)malloc(numBytes * h_res * v_res);
...
int drawPixel(uint16_t x, uint16_t y, uint32_t color) {
  if (x >= h_res || y >= v_res)
    return 0;
  else if (color == IGNORE_COLOR) //Predefined color to ignore
    return 0;

  char *tmp_mem = second_buffer;
...
void freeBuffers() { //Free video memory and second buffer
  free(video_mem);
  free(second_buffer);
}

void copyToVRAM() { //Copy second buffer to video memory
  video_mem = memcpy(video_mem, second_buffer, numBytes*h_res*v_res);
}
